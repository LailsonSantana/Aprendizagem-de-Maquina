...
for imagem in imagens:
    ...
    #Faz a normalização
    #imagem_array = np.array(imagem_suavizada)
    #imagem_normalizada = (imagem_array - imagem_array.min()) / (imagem_array.max() - imagem_array.min())
    #Salvar Imagem
    #imagem_desnormalizada = (imagem_normalizada * 255).astype(np.uint8)
    #imagem_pillow = Image.fromarray(imagem_desnormalizada)
    
    imagem_suavizada.save("imagem" + str(cont) + ".jpg")
